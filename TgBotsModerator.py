...
def handle_messages(message):
    ...
    # НЕ ПЕРЕСЫЛАЕМ ЕСЛИ:
    # 1. Это сообщение от канала (id 777000)
    # 2. Это пересланное сообщение
    # 3. Это сообщение содержит ссылку на канал
    if message.from_user.id == 777000:
        print("⏭️ Пропускаем системное сообщение")
        return
    
    if message.forward_from_chat:
        print("⏭️ Пропускаем пересланное сообщение")
        return
    
    text = message.text or ""
    
    # Проверяем что это не ссылка на канал
    if text and "t.me/" in text:
        print("⏭️ Пропускаем сообщение со ссылкой на канал")
        return
...
if __name__ == "__main__":
    ...
    print(f"📌 Обсуждение: {DISCUSSION_GROUP_ID}")
    print("=" * 50)
